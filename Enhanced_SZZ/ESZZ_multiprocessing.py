...
def get_commit_diff_string(commit, repo):
    ...
    try:
        ...
        if len(commit.parents) > 0:
            commit_diff = repo.git.diff('--unified=1', '--diff-filter=MRC', '--ignore-cr-at-eol', '--ignore-space-at-eol',
...
if __name__ == '__main__':
    ...
    # save labeled shas
    df_labeled_shas.to_csv( output_dir_path + "sha_label_eszz_1line.csv", index=False)
